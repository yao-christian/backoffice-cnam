...
import { RolesListSchema } from "./schemas/api.schemas";

import { Role } from "./role.type";
import { formatPaginatedData } from "@/components/utils/pagination";
import { ApiPaginationSchema } from "@/utils/api-utils";

export async function getRolesWithPagination({ page = 0, perPage = 10 }) {
  const apiPage = page + 1;
  const url = `${process.env.API_URL}/api/role/list?page=${apiPage}&limit=${perPage}`;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 404)
        throw new NotFoundError("Liste des roles introuvable.");
    ...
    if (responseData.status !== "success") {
      throw new HttpError(
        responseData.message ||
          "Impossible de récupérer les role. Veuillez réessayer plus tard.",
        422,
        "APPLICATION_ERROR",
      );
    }
    ...
    const roles = RolesListSchema.parse(parsedResponseData.data);

    const totalCount = parsedResponseData.total;
    const totalPages = parsedResponseData.lastPage;

    return formatPaginatedData<Role>({
      totalCount,
      totalPages,
      page,
