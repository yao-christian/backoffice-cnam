...
export async function GET(request: NextRequest) {
  ...
  try {
    // 2) Appel User (peut jeter HttpError / ValidationError)
    const data = await getUsersWithPagination({ page, perPage });

    // 3) Réponse succès
    return NextResponse.json(
      { status: "success", message: "Liste des utilisateurs", data },
