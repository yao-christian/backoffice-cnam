...
export async function GET(request: NextRequest) {
  ...
  try {
    // 2) Appel service (peut jeter HttpError / ValidationError)
    const data = await getPaymentNotificationsWithPagination({ page, perPage });

    // 3) Réponse succès
    return NextResponse.json(
      { status: "success", message: "Liste des cotisation", data },
      { headers: { "cache-control": "no-store" } },
    );
  } catch (error) {
    console.error("GET /api/cotisation error:", error);
