...
export function SaleList({ data }: PropsType) {
  const [dataState, setDataState] = useState(data);
  const [isLoading, setIsLoading] = useState(false);

  const columns = useMemo<ColumnDef<PaymentNotification>[]>(
    () => [
      {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      const res = await getPaymentNotifications({
        page: pageIndex,
        perPage: pageSize,
      });
