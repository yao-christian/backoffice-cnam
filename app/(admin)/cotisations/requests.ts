...
export const getPaymentNotifications = async ({ page, perPage }: GetParams) => {
  const url = `/api/payment-notifications?page=${page}&perPage=${perPage}`;
  ...
  if (status === "success") {
    return body.data as PaginatedData<PaymentNotification>;
  }
